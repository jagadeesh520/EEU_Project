...
const ServiceRequestStatus = ({navigation}) => {
  ...
  const clearData = () => {
    setAccountData({});
    setComplaintTitle('');
    setComplaintDescription(''), setSelectedType({});
  };
    ...
    fetch(constant.BASE_URL + constant.COMPLAINT_POST, {
      ...
      .then(async responseData => {
        if (responseData.Record[0].ComplaintNumber) {
          showToast(
            'success',
            'Complaint is raised successfully, Complaint Number:' +
  ...
  return (
    <ScrollView style={styles.DashBoardMain}>
                    ...
                    {data.isExpand && (
                      <View>
                        ...
                        <View style={styles.ComplaintListDesMain}>
                          <Text style={styles.ComplaintListHeader}>
                            {t('Complanit Resolution') + ' '}
                          </Text>
                          <Text style={styles.RaiseComplaintTxt}>
      ...
      ) : complaintData && complaintData.length == 0 && !isLoading ? (
        <View style={styles.ComplaintsMain}>
          <Image
            source={ImagePath.Complaint}
            style={styles.ComplaintsWaterMark}
          />
          <Text style={styles.ComplaintsCommonTxt}>
            {t('Great! No service request status in last')}
          </Text>
          ...
          <TouchableOpacity
            ...
            <Text style={styles.ComplaintsBtnTxt}>
              {t('RAISE A SERVICE REQUEST')}
            </Text>
          </TouchableOpacity>
        </View>
      ) : null}
      {isRaiseComplaint ? (
        <View>
          <View style={styles.RaiseComplaintMain}>
            <Text style={styles.RaiseComplaintDropdownTxt}>
              {t('Compliant Type')}
            </Text>
            <Dropdown
          ...
          <View style={styles.RaiseComplaintMain2}>
            ...
            <TouchableOpacity
              style={styles.RaiseComplaintBtn}
              onPress={() => {
                onPressSubmit(true);
              }}>
